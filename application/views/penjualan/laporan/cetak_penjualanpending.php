	...
	<table class="datatable3" style="width:150%" border="1">
		...
		<tbody>
			...
			foreach ($penjualan as $key => $p) {
				...
				$jmlbarang = $this->db->get_where('detailpenjualan_pending', array('no_fak_penj' => $p->no_fak_penj))->num_rows();
				...
				$query1    = "SELECT no_fak_penj,detailpenjualan_pending.kode_barang,nama_barang,jumlah,isipcsdus,
						  isipack,isipcs,detailpenjualan_pending.harga_dus,detailpenjualan_pending.harga_pack,detailpenjualan_pending.harga_pcs
						  ,subtotal,promo
						  FROM detailpenjualan_pending
						  INNER JOIN barang ON detailpenjualan_pending.kode_barang = barang.kode_barang
						  WHERE no_fak_penj = '$p->no_fak_penj' ORDER BY detailpenjualan_pending.kode_barang ASC LIMIT 1";
			...
			?>
				<tr>
					...
					<td style="background-color:<?php echo $warna; ?>" rowspan="<?php echo $jmlbarang; ?>"><?php echo $p->no_fak_penj; ?></td>
					...
					<td <?php if ($barang1['promo'] == 1) { ?> style="background-color: yellow" <?php } ?> align="right">
						<?php if ($jmldus != 0) {
							echo number_format($barang1['harga_dus'], '0', '', '.');
						} ?>
					</td>
					<td <?php if ($barang1['promo'] == 1) { ?> style="background-color: yellow" <?php } ?> align="center">
						<?php if ($jmlpack != 0) {
					...
					<td <?php if ($barang1['promo'] == 1) { ?> style="background-color: yellow" <?php } ?> align="right">
						<?php if ($jmlpack != 0) {
							echo number_format($barang1['harga_pack'], '0', '', '.');
						} ?>
					...
					<td <?php if ($barang1['promo'] == 1) { ?> style="background-color: yellow" <?php } ?> align="right">
						<?php if ($jmlpcs != 0) {
							echo number_format($barang1['harga_pcs'], '0', '', '.');
						} ?>
					</td>
					<td <?php if ($barang1['promo'] == 1) { ?> style="background-color: yellow" <?php } ?> align="right">
				...
				if ($jmlbarang > 1) {
					...
					$query2    = "SELECT no_fak_penj,detailpenjualan_pending.kode_barang,nama_barang,jumlah,isipcsdus,
						  isipack,isipcs,detailpenjualan_pending.harga_dus,detailpenjualan_pending.harga_pack,detailpenjualan_pending.harga_pcs
						  ,subtotal,promo
						  FROM detailpenjualan_pending
						  INNER JOIN barang ON detailpenjualan_pending.kode_barang = barang.kode_barang
						  WHERE no_fak_penj = '$p->no_fak_penj' ORDER BY detailpenjualan_pending.kode_barang ASC LIMIT 1,$jmlbarang";
